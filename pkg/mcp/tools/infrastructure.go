...
func (t *Toolsets) RegisterDeleteEnvironment(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Delete an environment from a namespace. This will remove the deployment target and any associated resources.",
		InputSchema: createSchema(map[string]any{
			"namespace_name": defaultStringProperty(),
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		InputSchema: createSchema(map[string]any{
			...
			"promotion_paths": map[string]any{
				"type":        "array",
				"description": "Promotion paths defining environment progression. Each path has a source environment and target environments with optional approval requirements",
...
func (t *Toolsets) RegisterGetResourceEvents(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		InputSchema: createSchema(map[string]any{
			"namespace_name":      defaultStringProperty(),
			"release_binding_name": stringProperty("Name of the release binding (deployment). Use list_release_bindings to discover valid names"),
...
// ---------------------------------------------------------------------------
// Component Toolset — ComponentType, Trait, Workflow (developer-facing read)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Component Toolset — Deployment pipeline (developer-facing read)
// ---------------------------------------------------------------------------
...
func (t *Toolsets) RegisterGetDeploymentPipeline(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		result, err := t.ComponentToolset.GetDeploymentPipeline(ctx, args.NamespaceName, args.PipelineName)
		return handleToolResult(result, err)
	})
}
...
func (t *Toolsets) RegisterListDeploymentPipelines(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		result, err := t.ComponentToolset.ListDeploymentPipelines(
			ctx, args.NamespaceName, ListOpts{Limit: args.Limit, Cursor: args.Cursor})
		return handleToolResult(result, err)
	})
}
...
// ---------------------------------------------------------------------------
// Component Toolset — WorkflowRun operations
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Component Toolset — Workflow read
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Component Toolset — Cluster-scoped resources
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Component Toolset — List/Get environments (developer-facing read, kept for
// backward compatibility when Component toolset is enabled without PE)
// ---------------------------------------------------------------------------
...
func (t *Toolsets) RegisterListEnvironments(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		result, err := t.ComponentToolset.ListEnvironments(
			ctx, args.NamespaceName, ListOpts{Limit: args.Limit, Cursor: args.Cursor})
		return handleToolResult(result, err)
	})
}
