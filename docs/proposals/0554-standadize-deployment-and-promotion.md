...
- UI/CLI: When developer triggers the deployment, the OpenChoreo API server will create the Release and ReleasePin for the first environment using Component, ComponentTypeDefinition, Addons, and Workload with embedded content. When developer triggers the promotion, the OpenChoreo API server will update the target environment's ReleasePin to previous environment's Release. The ReleasePin controller will reconcile the ReleasePin and create the ComponentEnvSnapshot for the target environment.
- GitOps: When developer do any update to the ReleasePin, the ReleasePin controller will reconcile the ReleasePin and create the ComponentEnvSnapshot for the target environment.
