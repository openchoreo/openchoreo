...
* The mechanism for injecting configurations and secrets into containers is defined within the ComponentTypeDefinition CR.
  * New custom CEL function (`oc_generate_name(parts...)`) is introduced for generating kubernetes names
    ...
    - id: deployment
      ...
      - id: file-config
          ...
          template:
            ...
            kind: ConfigMap
            metadata:
              name: ${oc_generate_name(metadata.name, "config", config.name)}
              namespace: ${metadata.namespace}
            data:
              ${config.name}: |
                ${config.value}
        ...
        - id: secret-file-external
          ...
          template:
            ...
            kind: ExternalSecret
            metadata:
              name: ${oc_generate_name(metadata.name, "secret", file.name)}
              namespace: ${metadata.namespace}
            spec:
              refreshInterval: 15s
              secretStoreRef:
