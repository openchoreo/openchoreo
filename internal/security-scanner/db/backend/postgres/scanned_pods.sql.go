...
func (q *Queries) DeleteScannedPod(ctx context.Context, id int32) error {
	_, err := q.db.ExecContext(ctx, deleteScannedPod, id)
	return err
}

const getScannedPod = `-- name: GetScannedPod :one
SELECT id, pod_name FROM scanned_pods WHERE id = $1
`

func (q *Queries) GetScannedPod(ctx context.Context, id int32) (ScannedPod, error) {
...
SELECT id, pod_name FROM scanned_pods WHERE pod_name = $1
`
