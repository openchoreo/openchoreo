...
func (q *Queries) DeletePostureFindingsByResourceID(ctx context.Context, resourceID int32) error {
...
SELECT id, resource_id, check_id, check_name, severity, category, description, remediation, resource_version, created_at FROM posture_findings WHERE resource_id = $1 ORDER BY created_at DESC
`

func (q *Queries) GetPostureFindingsByResourceID(ctx context.Context, resourceID int32) ([]PostureFinding, error) {
...
type UpsertPostureScannedResourceParams struct {
	ResourceID      int32         `json:"resource_id"`
	ResourceVersion string        `json:"resource_version"`
	ScanDurationMs  sql.NullInt32 `json:"scan_duration_ms"`
}
