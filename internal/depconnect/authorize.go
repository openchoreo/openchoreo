...
// AuthorizeRequest is what the dep-agent POSTs to the control plane's authorize
...
// AuthorizePath is the control-plane route the dep-agent calls to authorize a stream.
const AuthorizePath = "/api/v1/dep-connect:authorize"

// HeartbeatRequest is what a dep-agent POSTs periodically while it has at least one
...
// HeartbeatPath is the control-plane route a dep-agent calls to refresh its liveness.
const HeartbeatPath = "/api/v1/dep-connect:heartbeat"
