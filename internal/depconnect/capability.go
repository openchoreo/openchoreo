...
// CapabilityAudience is the JWT audience for dep-connect capabilities — a
// defense-in-depth check against token reuse across unrelated JWT purposes.
const CapabilityAudience = "openchoreo-api:dep-connect"
...
// and signs them here, so the dep-connect stream endpoint dials only CP-authorized
...
type Target struct {
	...
	// AgentNamespace is the data-plane namespace of the dep-agent that serves this
...
// CapabilityClaims are the custom claims of the dep-connect capability JWT. The
...
// the dep-connect audience. On success it returns the claims.
func VerifyCapability(token string, pub ed25519.PublicKey) (*CapabilityClaims, error) {
	...
	if _, err := parser.ParseWithClaims(token, claims, func(t *jwt.Token) (any, error) {
		...
		return nil, fmt.Errorf("depconnect: verify capability: %w", err)
	}
	return claims, nil
}
...
func VerifyCapabilityAllowExpired(token string, pub ed25519.PublicKey) (*CapabilityClaims, error) {
	...
	if !slices.Contains(claims.Audience, CapabilityAudience) {
		return nil, fmt.Errorf("depconnect: capability audience mismatch")
	}
	// WithoutClaimsValidation drops every temporal check, not only expiry; re-assert the
	// two that still apply.
	if claims.ExpiresAt == nil {
		return nil, fmt.Errorf("depconnect: capability has no expiry")
	}
	if claims.NotBefore != nil && claims.NotBefore.After(time.Now()) {
		return nil, fmt.Errorf("depconnect: capability is not valid yet")
	}
	return claims, nil
}
