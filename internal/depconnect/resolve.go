...
// ResolveRequest is what occ sends to the control plane's dep-connect resolve
...
// the CP-signed capability occ presents to the dep-agents, and the set of dep-agents occ
// dials directly. Each target names (AgentID) the dep-agent that serves it; the control
// plane provisions one dep-agent per provider project+env data-plane namespace on resolve,
...
type ResolveResponse struct {
	...
	// Agents maps an agent ID (the provider's data-plane namespace) to the dep-agent occ
...
// AgentEndpoint is one dep-agent occ dials over TLS (layering yamux on top). With the
...
type AgentEndpoint struct {
	// Endpoint is the "host:port" occ dials — the shared dep-connect SNI router.
...
type ResolvedTarget struct {
	...
	// AgentID indexes ResolveResponse.Agents: the dep-agent occ opens this target's
...
// dep-agent looks up in the capability to find its dial target. The provider project is
