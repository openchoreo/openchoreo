...
const (
	...
	// of 1. The resource and workflow pipelines were measured afterwards: their heaviest
	// whole reconcile is 4,351, three orders of magnitude below this limit, so neither
	// constrains it.
	//
	// The measurements are upstream's, taken on a tree that also had a project pipeline,
	// which does not exist on this branch; nothing here was re-measured. The ext-overload
	// trackers in ext_overload_costs.go can only raise a measured cost, so the headroom
	// figures above are upper bounds rather than current readings.
	//
...
// upstream measured across all three pipelines: ceil((Rmax_global x 10) /
// defaultCELCostLimit) = ceil(694,370 / 2,000,000) = 1. Rmax_global is 69,437, the
// component pipeline's heaviest whole render. Whole reconciles of the resource and workflow
// pipelines, each driven through its real entry-point sequence on one shared budget, top
// out at 4,351. The resulting budget of 2,000,000 leaves 28.8x headroom. See
