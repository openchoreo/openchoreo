...
func TestGenerateSelfSignedCert(t *testing.T) {
	certPEM, keyPEM, err := generateSelfSignedCert("myproj-dev.dep-connect", time.Now())
	if err != nil {
	...
	if len(cert.DNSNames) != 1 || cert.DNSNames[0] != "myproj-dev.dep-connect" {
		t.Fatalf("unexpected SANs: %v", cert.DNSNames)
	}
	// occ pins the cert and verifies against the SNI it was told to use.
	pool := x509.NewCertPool()
	pool.AddCert(cert)
	if _, err := cert.Verify(x509.VerifyOptions{DNSName: "myproj-dev.dep-connect", Roots: pool}); err != nil {
...
func TestEnsureAgentProvisionsClusterIPWithSNI(t *testing.T) {
	cfg := config.DepConnectConfig{
		AgentImage:        "ghcr.io/openchoreo/dep-agent:test",
		AgentListenPort:   8443,
		EntrypointAddress: "depconnect.example.com:8443",
		SNISuffix:         "dep-connect",
		AuthorizeURL:      "https://api.example.com/api/v1/dep-connect:authorize",
	}
	p := newDepAgentProvisioner(cfg, slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	wantSNI := ns + ".dep-connect"
	if info.endpoint != "depconnect.example.com:8443" {
	...
	svc := &corev1.Service{}
	if err := cl.Get(context.Background(), client.ObjectKey{Namespace: ns, Name: depAgentName}, svc); err != nil {
...
func TestAgentCertIsNotACA(t *testing.T) {
	const sni = "myproj-dev.dep-connect"
...
func storedPair(t *testing.T, cl client.Client, ns string) (cert, key []byte) {
	...
	if err := cl.Get(context.Background(), client.ObjectKey{Namespace: ns, Name: depAgentName}, secret); err != nil {
...
func certTestProvisioner(now time.Time) *depAgentProvisioner {
	p := newDepAgentProvisioner(config.DepConnectConfig{
		AgentImage: "img", AgentListenPort: 8443, EntrypointAddress: "e:8443", SNISuffix: "dep-connect",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "cert for a different SNI is reissued":
				c, k, err := generateSelfSignedCert("someone-else.dep-connect", now)
				if err != nil {
					t.Fatal(err)
				}
				raw, rawKey = c, k
...
func TestEnsureAgentDeploymentIsHardened(t *testing.T) {
	...
	if err := cl.Get(context.Background(), client.ObjectKey{Namespace: ns, Name: depAgentName}, dep); err != nil {
...
func TestApplyDeploymentHeartbeatURLDerivation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			authorizeURL:  "https://api.example.com" + depconnect.AuthorizePath,
			wantHeartbeat: "--heartbeat-url=https://api.example.com" + depconnect.HeartbeatPath,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := newDepAgentProvisioner(config.DepConnectConfig{
				...
				SNISuffix: "dep-connect", AuthorizeURL: tt.authorizeURL,
			}, slog.New(slog.NewTextHandler(io.Discard, nil)))
			cl := fake.NewClientBuilder().WithScheme(provScheme(t)).Build()
			const ns = "dp-default-proj-development"
