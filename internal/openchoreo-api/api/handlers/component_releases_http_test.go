...
// HTTP-layer integration tests for the component releases resource.
//
// These tests go beyond the direct-call tests in component_releases_test.go by
...
//
//  3. K8s side effects — create/delete operations verify the expected object state
//     in the fake client after the HTTP call returns, confirming that the handler
//     actually mutates the store rather than just returning the right status code.
