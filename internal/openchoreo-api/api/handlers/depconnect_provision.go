...
const (
	// depAgentName is the fixed name of the dep-agent Deployment/Service/Secret within
	// a project+env data-plane namespace (the namespace already scopes it, so one
	// fixed name per namespace is unique).
	depAgentName = "openchoreo-dep-agent"
	// sniAnnotationKey holds each dep-agent Service's SNI host; the shared dep-connect
	// SNI router reads it to route occ's connection to this agent. Must match the
	// router's --sni-annotation (depconnectrouter.DefaultSNIAnnotationKey).
	sniAnnotationKey = "openchoreo.dev/depconnect-sni"
	// defaultSNISuffix is appended to the data-plane namespace to form an agent's SNI
	// host when dep_connect.sni_suffix is unset.
	defaultSNISuffix = "dep-connect"
	// depAgentFieldOwner is the server-side-apply field manager for provisioned resources.
	depAgentFieldOwner = "openchoreo-api-depconnect"
	// lastUsedAnnotation records the last time a resolve refreshed this dep-agent; the
	// reaper deletes agents idle past the TTL.
	lastUsedAnnotation = "openchoreo.dev/depconnect-last-used"
	// managedByLabelValue marks resources the dep-connect provisioner owns.
	managedByLabelValue = "openchoreo-api-depconnect"
	...
	certAnnotation = "openchoreo.dev/depconnect-cert"
...
// agentEndpointInfo is what a provisioned dep-agent exposes to occ.
...
// depAgentProvisioner imperatively creates/updates a per-project+env dep-agent
...
type depAgentProvisioner struct {
	cfg    config.DepConnectConfig
	now    func() time.Time
	logger *slog.Logger
}

func newDepAgentProvisioner(cfg config.DepConnectConfig, logger *slog.Logger) *depAgentProvisioner {
	return &depAgentProvisioner{cfg: cfg, now: time.Now, logger: logger.With("component", "dep-connect-provisioner")}
}
...
func (p *depAgentProvisioner) agentSNI(dpNamespace string) string {
...
// ensureAgent applies the dep-agent resources (Deployment + ClusterIP Service + cert
...
func (p *depAgentProvisioner) ensureAgent(ctx context.Context, dpClient client.Client, dpNamespace string) (*agentEndpointInfo, error) {
	if p.cfg.EntrypointAddress == "" {
		return nil, fmt.Errorf("dep_connect.entrypoint_address is not configured")
	}
	sni := p.agentSNI(dpNamespace)

	certPEM, err := p.ensureCertSecret(ctx, dpClient, dpNamespace, sni)
	if err != nil {
		return nil, fmt.Errorf("ensure dep-agent cert: %w", err)
	}
	if err := p.applyDeployment(ctx, dpClient, dpNamespace, certPEM); err != nil {
		return nil, fmt.Errorf("apply dep-agent deployment: %w", err)
	}
	if err := p.applyService(ctx, dpClient, dpNamespace, sni); err != nil {
		return nil, fmt.Errorf("apply dep-agent service: %w", err)
	}
...
func (p *depAgentProvisioner) ensureCertSecret(ctx context.Context, dpClient client.Client, ns, sni string) (string, error) {
	secret := &corev1.Secret{}
	getErr := dpClient.Get(ctx, client.ObjectKey{Namespace: ns, Name: depAgentName}, secret)
	if getErr == nil {
...
func (p *depAgentProvisioner) storedCert(ctx context.Context, dpClient client.Client, ns string) (string, error) {
	secret := &corev1.Secret{}
	if err := dpClient.Get(ctx, client.ObjectKey{Namespace: ns, Name: depAgentName}, secret); err != nil {
		return "", err
	}
	cert := secret.Data[corev1.TLSCertKey]
	if len(cert) == 0 {
		return "", fmt.Errorf("dep-agent cert secret in %s has no %s", ns, corev1.TLSCertKey)
	}
	return string(cert), nil
}

func (p *depAgentProvisioner) applyDeployment(ctx context.Context, dpClient client.Client, ns string, certPEM string) error {
	replicas := int32(1)
	...
	if hb := strings.Replace(p.cfg.AuthorizeURL, depconnect.AuthorizePath, depconnect.HeartbeatPath, 1); hb != p.cfg.AuthorizeURL {
	...
	dep := &appsv1.Deployment{
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: &replicas,
			Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"app": depAgentName}},
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Volumes: []corev1.Volume{{
						Name:         "certs",
						VolumeSource: corev1.VolumeSource{Secret: &corev1.SecretVolumeSource{SecretName: depAgentName}},
					}},
				},
			},
		},
	}
	return dpClient.Patch(ctx, dep, client.Apply, client.ForceOwnership, client.FieldOwner(depAgentFieldOwner))
}

func (p *depAgentProvisioner) applyService(ctx context.Context, dpClient client.Client, ns, sni string) error {
	port := int32(p.cfg.AgentListenPort) //nolint:gosec // config-bounded port
	...
	svc := &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			Selector: map[string]string{"app": depAgentName},
	...
	return dpClient.Patch(ctx, svc, client.Apply, client.ForceOwnership, client.FieldOwner(depAgentFieldOwner))
}

// touchLastUsed refreshes the dep-agent's last-used annotation so the reaper keeps it
...
func (p *depAgentProvisioner) touchLastUsed(ctx context.Context, dpClient client.Client, ns string) error {
	patch := fmt.Sprintf(`{"metadata":{"annotations":{%q:%q}}}`,
		lastUsedAnnotation, p.now().UTC().Format(time.RFC3339))
	dep := &appsv1.Deployment{
		TypeMeta:   metav1.TypeMeta{APIVersion: "apps/v1", Kind: "Deployment"},
		ObjectMeta: metav1.ObjectMeta{Name: depAgentName, Namespace: ns},
	}
	return dpClient.Patch(ctx, dep, client.RawPatch(types.MergePatchType, []byte(patch)))
}

func (p *depAgentProvisioner) objectMeta(ns string) metav1.ObjectMeta {
...
func (p *depAgentProvisioner) labelSet() map[string]string {
	return map[string]string{
		"app":                          depAgentName,
		"app.kubernetes.io/managed-by": managedByLabelValue,
		labels.LabelKeySystemComponent: depAgentName,
	}
}

// generateSelfSignedCert produces a PEM cert/key pair with serverName as its SAN,
// used by the dep-agent as its TLS server cert and pinned by occ.
...
func (p *depAgentProvisioner) certUsable(certPEM, keyPEM []byte, sni string) bool {
