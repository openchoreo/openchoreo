...
// DepConnectHeartbeatHandler serves POST /api/v1/dep-connect:heartbeat — the periodic
// liveness callback a dep-agent makes while it has at least one live tunnel session.
...
type DepConnectHeartbeatHandler struct {
	verifyKey ed25519.PublicKey
	// touch refreshes the named dep-agent's last-used annotation.
...
// NewDepConnectHeartbeatHandler builds the heartbeat handler. verifyKey is the public
// half of the key DepConnectHandler signs capabilities with; touch refreshes the
// dep-agent's liveness annotation.
func NewDepConnectHeartbeatHandler(verifyKey ed25519.PublicKey, touch func(ctx context.Context, namespace, env, dpNamespace string) error, logger *slog.Logger) *DepConnectHeartbeatHandler {
	return &DepConnectHeartbeatHandler{
		verifyKey: verifyKey,
		touch:     touch,
		logger:    logger.With("component", "dep-connect-heartbeat-handler"),
	}
}

func (h *DepConnectHeartbeatHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var req depconnect.HeartbeatRequest
	...
	claims, err := depconnect.VerifyCapabilityAllowExpired(req.Capability, h.verifyKey)
	if err != nil {
