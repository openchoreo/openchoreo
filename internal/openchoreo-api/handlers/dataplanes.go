...
// UpdateDataPlaneGateway handles PATCH /api/v1/namespaces/{namespaceName}/dataplanes/{dpName}
// Only the gateway spec is mutable; all other fields are immutable after creation.
func (h *Handler) UpdateDataPlaneGateway(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	namespaceName := r.PathValue("namespaceName")
	dpName := r.PathValue("dpName")
	...
	var req models.UpdateDataPlaneGatewayRequest
	...
	setAuditResource(ctx, "dataplane", dpName, dpName)
	addAuditMetadata(ctx, "organization", namespaceName)

	dataplane, err := h.services.DataPlaneService.UpdateDataPlaneGateway(ctx, namespaceName, dpName, &req)
	if err != nil {
		if errors.Is(err, services.ErrForbidden) {
			h.logger.Warn("Unauthorized to update dataplane", "namespace", namespaceName, "dataplane", dpName)
			writeErrorResponse(w, http.StatusForbidden, services.ErrForbidden.Error(), services.CodeForbidden)
			return
		}
		if errors.Is(err, services.ErrDataPlaneNotFound) {
		...
		h.logger.Error("Failed to update dataplane gateway", "error", err, "namespace", namespaceName, "dataplane", dpName)
		writeErrorResponse(w, http.StatusInternalServerError, "Failed to update dataplane gateway", services.CodeInternalError)
		return
	}
...
// DeleteDataPlane handles DELETE /api/v1/namespaces/{namespaceName}/dataplanes/{dpName}
func (h *Handler) DeleteDataPlane(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	namespaceName := r.PathValue("namespaceName")
	dpName := r.PathValue("dpName")
	...
	if err := h.services.DataPlaneService.DeleteDataPlane(ctx, namespaceName, dpName); err != nil {
		if errors.Is(err, services.ErrForbidden) {
			h.logger.Warn("Unauthorized to delete dataplane", "namespace", namespaceName, "dataplane", dpName)
			writeErrorResponse(w, http.StatusForbidden, services.ErrForbidden.Error(), services.CodeForbidden)
			return
		}
		if errors.Is(err, services.ErrDataPlaneNotFound) {
		...
		h.logger.Error("Failed to delete dataplane", "error", err, "namespace", namespaceName, "dataplane", dpName)
		writeErrorResponse(w, http.StatusInternalServerError, "Failed to delete dataplane", services.CodeInternalError)
		return
	}
