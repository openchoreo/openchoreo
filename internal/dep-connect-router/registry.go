...
// registry maps a dep-agent's SNI host to its in-cluster backend address, discovered
// by listing the per-project+env dep-agent Services the control plane provisions
...
// refresh rebuilds the SNI->backend map from the current dep-agent Services.
func (r *registry) refresh(ctx context.Context) {
	...
	for i := range list.Items {
		...
		if existing, dup := next[sni]; dup {
			if existing != backend {
				r.log.Warn("duplicate dep-connect SNI; ignoring", "sni", sni, "kept", existing, "ignored", backend)
			}
			continue
		}
		next[sni] = backend
