...
// startTLSEchoBackend stands in for a dep-agent: it terminates TLS for sni and echoes.
// A successful handshake proves the router replayed the ClientHello byte-exact.
func startTLSEchoBackend(t *testing.T, sni string) (addr string, pool *x509.CertPool) {
	t.Helper()
	cert, pool := selfSignedCert(t, sni)
...
func TestRouterRoutesBySNIAndSplices(t *testing.T) {
	backendAddr, pool := startTLSEchoBackend(t, testSNI)
...
func TestRouterDropsUnknownSNI(t *testing.T) {
	_, pool := startTLSEchoBackend(t, testSNI)
