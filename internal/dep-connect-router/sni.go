...
var errNoSNI = errors.New("depconnectrouter: no SNI in ClientHello")
...
func readClientHelloSNI(r io.Reader) (sni string, raw []byte, err error) {
	...
	if _, err := io.ReadFull(r, hdr[:]); err != nil {
		return "", nil, fmt.Errorf("depconnectrouter: read record header: %w", err)
	}
	recordType := hdr[0]
	if recordType != 0x16 { // handshake
		return "", nil, fmt.Errorf("depconnectrouter: not a TLS handshake record (type %d)", recordType)
	}
	recLen := int(binary.BigEndian.Uint16(hdr[3:5]))
	if recLen == 0 || recLen > maxClientHelloRecord {
		return "", nil, fmt.Errorf("depconnectrouter: bad ClientHello record length %d", recLen)
	}
	...
	if _, err := io.ReadFull(r, body); err != nil {
		return "", nil, fmt.Errorf("depconnectrouter: read record body: %w", err)
	}
...
func parseClientHelloSNI(b []byte) (string, error) {
	...
	if len(b) < 4 || b[0] != 0x01 { // ClientHello
		return "", errors.New("depconnectrouter: not a ClientHello")
	}
	b = b[4:]
