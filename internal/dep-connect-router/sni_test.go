...
func TestReadClientHelloSNI(t *testing.T) {
	...
	go func() {
		c := tls.Client(clientConn, &tls.Config{ServerName: "myproj-dev.dep-connect", InsecureSkipVerify: true}) //nolint:gosec // test
	...
	if sni != "myproj-dev.dep-connect" {
		t.Fatalf("sni = %q, want myproj-dev.dep-connect", sni)
	}
