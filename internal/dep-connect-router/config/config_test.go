...
// testFlags mirrors the flag set cmd/dep-connect-router registers, so the mappings under
// test are the ones the binary actually uses.
func testFlags(t *testing.T, args ...string) *pflag.FlagSet {
	t.Helper()
	flags := pflag.NewFlagSet("dep-connect-router", pflag.ContinueOnError)
...
func TestDefaults(t *testing.T) {
	...
	assert.Equal(t, depconnectrouter.DefaultLabelSelector, cfg.Router.LabelSelector)
	assert.Equal(t, depconnectrouter.DefaultSNIAnnotationKey, cfg.Router.SNIAnnotationKey)
	assert.Equal(t, depconnectrouter.DefaultRefreshInterval, cfg.Router.RefreshInterval)
	assert.Equal(t, depconnectrouter.DefaultDialTimeout, cfg.Router.DialTimeout)
...
`), 0o600))
	...
	t.Setenv("OC_DEP_ROUTER__ROUTER__LISTEN_ADDR", ":7443")
	t.Setenv("OC_DEP_ROUTER__LOGGING__LEVEL", "debug")
...
func TestToRouterConfig(t *testing.T) {
	...
	assert.Equal(t, depconnectrouter.Config{
		ListenAddr:       ":9443",
