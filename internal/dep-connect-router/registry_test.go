...
const (
	testSNI       = "dp-doclet-development.dep-connect"
	testAgentName = "openchoreo-dep-agent"
	testBackend   = "openchoreo-dep-agent.dp-doclet-development.svc.cluster.local:8443"
)
...
// agentService builds a dep-agent Service as the control-plane provisioner creates it:
...
func agentService(ns, sni string, port int32) *corev1.Service {
	svc := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      testAgentName,
			Namespace: ns,
			Labels:    map[string]string{"app.kubernetes.io/managed-by": "openchoreo-api-depconnect"},
		},
	}
	if sni != "" {
...
func TestRegistryRefreshBuildsBackendMap(t *testing.T) {
	unlabeled := agentService("dp-other-development", "unlabeled.dep-connect", 8443)
	...
	reg, _ := newTestRegistry(
		...
		agentService("dp-noport-development", "noport.dep", 0), // no ports
		unlabeled, // not managed by dep-connect
		agentService("dp-second-development", "second.dep", 9443), // a second real agent
	)
	...
	if want := "openchoreo-dep-agent.dp-second-development.svc.cluster.local:9443"; got["second.dep"] != want {
		t.Errorf("byHost[second.dep] = %q, want %q", got["second.dep"], want)
	}
	if len(got) != 2 {
		t.Errorf("expected only the 2 annotated+labeled+ported agents to be routable, got %v", got)
	}
	if _, ok := got["unlabeled.dep-connect"]; ok {
		t.Error("a Service without the managed-by label must not be routable")
	}
	if _, ok := got["noport.dep"]; ok {
...
func TestRegistryLookupUnknownSNI(t *testing.T) {
	reg, _ := newTestRegistry(agentService("dp-doclet-development", testSNI, 8443))
	if addr, ok := reg.lookup(context.Background(), "nobody.dep-connect"); ok {
...
func TestRegistryDuplicateSNIIsDeterministic(t *testing.T) {
	reg, _ := newTestRegistry(
		agentService("dp-a-development", "shared.dep-connect", 8443),
		agentService("dp-b-development", "shared.dep-connect", 8443),
	)

	reg.refresh(context.Background())
	first := reg.byHostSnapshot()["shared.dep-connect"]
	if first == "" {
	...
	for range 5 {
		reg.refresh(context.Background())
		if got := reg.byHostSnapshot()["shared.dep-connect"]; got != first {
...
func TestRegistryMissRefreshIsRateLimited(t *testing.T) {
	...
	for range 50 {
		...
		go func() {
			defer wg.Done()
			reg.lookup(context.Background(), "nobody.dep-connect")
		}()
	}
	wg.Wait()
