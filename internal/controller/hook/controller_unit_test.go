...
var _ = Describe("Hook Controller (fake client)", func() {
	var (
		key    = types.NamespacedName{Name: "scan", Namespace: "default"}
		errAPI = errors.New("api down")
	)

	newHook := func(generation int64, conds ...metav1.Condition) *openchoreov1alpha1.Hook {
		return &openchoreov1alpha1.Hook{
			ObjectMeta: metav1.ObjectMeta{Name: key.Name, Namespace: key.Namespace, Generation: generation},
	...
	newReconciler := func(funcs *interceptor.Funcs, objs ...client.Object) (*Reconciler, client.Client) {
		b := fake.NewClientBuilder().WithScheme(scheme.Scheme).
			WithObjects(objs...).WithStatusSubresource(&openchoreov1alpha1.Hook{})
		if funcs != nil {
	...
	// A Hook deleted before its event is processed must not fail the reconcile,
	// or the workqueue retries a Hook that no longer exists.
	It("ignores a Hook that no longer exists", func() {
		r, _ := newReconciler(nil)
		Expect(reconcileHook(r)).To(Succeed())
	})

	// Any other read failure must be returned so the request is retried.
	// Swallowing it would leave the Hook without Available.
	...
	// A terminating Hook must not be marked Available. Doing so would tell
	// pipeline authors they can still bind a Hook that is being removed.
	It("leaves a terminating Hook untouched", func() {
		h := newHook(1)
	...
	// An up-to-date Hook must not be written again. Every write triggers another
	...
	It("skips the status write when already Available at the current generation", func() {
		...
		r, _ := newReconciler(&interceptor.Funcs{
			SubResourceUpdate: func(context.Context, client.Client, string, client.Object, ...client.SubResourceUpdateOption) error {
				Fail("status must not be written for an up-to-date Hook")
				return nil
			},
		}, h)
		Expect(reconcileHook(r)).To(Succeed())
	...
	// A failed status write must be returned so the Hook is requeued rather
	...
	// Without this registration the manager never reconciles Hooks, so none of
