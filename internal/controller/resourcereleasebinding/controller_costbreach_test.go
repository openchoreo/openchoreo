...
var _ = Describe("ResourceReleaseBinding controller — CEL cost breach", func() {
	...
	// rather than the per-expression limit: 250 distinct expressions, each about
	// ten thousand cost units — comfortably legal on its own — that together
	...
	// message-determinism assertion below is not vacuous. The cost comes mostly
	// from the comprehension: cel-go v0.22.1 charges a bare lists.range O(1),
	// and the ext-overload trackers add only the range's own length on top.
	budgetBurningTemplate := func(name string) *runtime.RawExtension {
		data := make(map[string]string, 250)
		for i := range 250 {
			data[fmt.Sprintf("field%03d", i)] = fmt.Sprintf("${size(lists.range(%d).map(j, j + 1))}", 700+i)
		}
		body := map[string]any{
			"apiVersion": "v1",
			"kind":       "ConfigMap",
