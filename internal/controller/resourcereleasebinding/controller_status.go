...
func (r *Reconciler) evaluateResourcesReady(
	...
	for i := range entries {
		...
		if entry.ReadyWhen != "" {
			// Each readyWhen is bounded on its own by the per-expression cost limit, and
			// all of them draw on the one budget seeded for this reconcile, so a long
			// resource list cannot spend more in total than a single render is allowed.
