...
// ComponentComponentReleaseBindings and extracting endpoint URLs from their status.
...
// dependency ComponentReleaseBinding and extracting the endpoint URL from its status.
...
func (r *Reconciler) resolveConnection(
	...
	indexKey := controller.MakeComponentReleaseBindingOwnerEnvKey(conn.Project, conn.Component, conn.Environment)

	var rbList openchoreov1alpha1.ComponentReleaseBindingList
	if err := r.List(ctx, &rbList,
		client.InNamespace(conn.Namespace),
		client.MatchingFields{controller.IndexKeyComponentReleaseBindingOwnerEnv: indexKey}); err != nil {
		return nil, nil, fmt.Errorf("failed to list ComponentComponentReleaseBindings for component %s/%s: %w", conn.Project, conn.Component, err)
	}
	...
	if len(rbList.Items) == 0 {
		return nil, &openchoreov1alpha1.PendingConnection{
			...
			Reason:    fmt.Sprintf("ComponentReleaseBinding not found for component %s/%s", conn.Project, conn.Component),
		}, nil
	}
	...
	if len(rbList.Items) > 1 {
		return nil, &openchoreov1alpha1.PendingConnection{
			...
			Component: conn.Component,
			Endpoint:  conn.Endpoint,
			Reason:    fmt.Sprintf("multiple ComponentComponentReleaseBindings found for component %s/%s in environment %s", conn.Project, conn.Component, conn.Environment),
		}, nil
	}
...
// each carrying its own pre-computed env vars from the resolved connections in ComponentReleaseBinding status.
...
// setConnectionsCondition sets the ConnectionsResolved condition on the ComponentReleaseBinding.
