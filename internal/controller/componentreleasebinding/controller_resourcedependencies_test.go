...
func TestBuildResourceDependencyTargets(t *testing.T) {
	...
	t.Run("uses_consumers_namespace_project_environment", func(t *testing.T) {
		...
		// environment all come from the consuming ComponentReleaseBinding, not from the dep itself.
...
// consumer ComponentReleaseBinding's workload deps must produce the same key that the index extracts
...
func TestSetupResourceDependencyTargetsIndex_indexerEmitsOneKeyPerTarget(t *testing.T) {
	// Build a ComponentReleaseBinding with two distinct resource-dep targets and verify the
...
func TestFindConsumerComponentComponentReleaseBindingsForResourceReleaseBinding(t *testing.T) {
	...
	c := fake.NewClientBuilder().
		...
		WithIndex(&openchoreov1alpha1.ComponentReleaseBinding{},
	...
	got := r.findConsumerComponentComponentReleaseBindingsForResourceReleaseBinding(context.Background(), provider)
	require.Len(t, got, 1, "expected one consumer enqueued")
	assert.Equal(t, "consumer-rb", got[0].Name)
}

func TestFindConsumerComponentComponentReleaseBindingsForResourceReleaseBinding_noConsumers(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, openchoreov1alpha1.AddToScheme(scheme))
	c := fake.NewClientBuilder().
		WithScheme(scheme).
...
func TestFindConsumerComponentComponentReleaseBindingsForResourceReleaseBinding_wrongType(t *testing.T) {
	r := newResourceDepReconciler(t)
	got := r.findConsumerComponentComponentReleaseBindingsForResourceReleaseBinding(context.Background(),
		&openchoreov1alpha1.ComponentReleaseBinding{}) // wrong type
	assert.Nil(t, got)
}

func TestFindConsumerComponentComponentReleaseBindingsForResourceReleaseBinding_skipsMalformedRRB(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := r.findConsumerComponentComponentReleaseBindingsForResourceReleaseBinding(context.Background(), c.rrb)
			assert.Nil(t, got)
		})
	}
}
...
// indexResourceDependencyTargets and findConsumerComponentComponentReleaseBindingsForResourceReleaseBinding's
...
func TestResourceReleaseBindingOutputsChangedPredicate(t *testing.T) {
	pred := resourceComponentReleaseBindingOutputsChangedPredicate()
	...
	t.Run("rejects_wrong_type", func(t *testing.T) {
		...
		assert.False(t, pred.Update(event.UpdateEvent{
			ObjectOld: &openchoreov1alpha1.ComponentReleaseBinding{},
			ObjectNew: &openchoreov1alpha1.ComponentReleaseBinding{},
		}))
	})
}
...
func newRBForResourceDeps(namespace, project, component, environment string) *openchoreov1alpha1.ComponentReleaseBinding {
