...
func bindingFor(ns, name, environment string) *openchoreov1alpha1.ComponentReleaseBinding {
	return &openchoreov1alpha1.ComponentReleaseBinding{
		ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: name},
		Spec:       openchoreov1alpha1.ComponentReleaseBindingSpec{Environment: environment},
	}
}
...
// TestFindComponentComponentReleaseBindingsForDataPlane: a namespaced DataPlane change enqueues only the bindings
// whose target environment references that DataPlane by matching kind+name, within its namespace.
func TestFindComponentComponentReleaseBindingsForDataPlane(t *testing.T) {
	...
	t.Run("matches_only_bindings_for_that_dataplane", func(t *testing.T) {
		reqs := r.findComponentComponentReleaseBindingsForDataPlane(ctx,
	...
	t.Run("environment_without_binding_yields_nothing", func(t *testing.T) {
		...
		reqs := r2.findComponentComponentReleaseBindingsForDataPlane(ctx,
			&openchoreov1alpha1.DataPlane{ObjectMeta: metav1.ObjectMeta{Namespace: "org", Name: "dp-x"}})
		assert.Empty(t, reqs)
	})
}

// TestFindComponentComponentReleaseBindingsForClusterDataPlane: a cluster-scoped DataPlane change scans every
...
func TestFindComponentComponentReleaseBindingsForClusterDataPlane(t *testing.T) {
	ctx := context.Background()
	r := newReconcilerWith(t,
	...
	reqs := r.findComponentComponentReleaseBindingsForClusterDataPlane(ctx,
