...
// providerRBFixture builds a provider ComponentReleaseBinding that is discoverable through
...
func providerRBFixture(name, project, component, envName string) *openchoreov1alpha1.ComponentReleaseBinding {
...
func setProviderEndpoints(rb *openchoreov1alpha1.ComponentReleaseBinding, endpoints []openchoreov1alpha1.EndpointURLStatus) {
	GinkgoHelper()
	fetched := &openchoreov1alpha1.ComponentReleaseBinding{}
...
func hasPendingReason(rb *openchoreov1alpha1.ComponentReleaseBinding, substr string) bool {
...
// These specs exercise connection resolution from the consumer ComponentReleaseBinding's point of view —
// it is the only RB that reconciles. For each dependency the reconcile records a ConnectionTarget
// (defaulting an empty Project to the consumer's project), then looks up the provider ComponentReleaseBinding
...
//
// The provider ComponentComponentReleaseBindings are passive fixtures. Reconciliation is driven manually and only the
...
var _ = Describe("ComponentReleaseBinding connection resolution", func() {
	// S1: no provider ComponentReleaseBinding exists for the connection's owner+env. The consumer still
	...
	Context("S1: when the provider ComponentReleaseBinding is not found", func() {
		const (
			rbName   = "rb-conn-s1"
			crName   = "cr-conn-s1"
			envName  = "env-conn-s1"
		...
		It("surfaces a pending connection with default-project fallback and blocks ReleaseSynced", func() {
			...
			By("Creating consumer ComponentReleaseBinding")
			...
			rb := reconcileUntil(r, rbName, func(rb *openchoreov1alpha1.ComponentReleaseBinding) bool {
				return hasPendingReason(rb, "ComponentReleaseBinding not found")
			})
			...
			Expect(rb.Status.PendingConnections[0].Reason).To(ContainSubstring("ComponentReleaseBinding not found"))
	...
	// S2: two provider ComponentComponentReleaseBindings share the same owner+env index key, so the consumer's
	// lookup is ambiguous. It refuses to pick one and marks the connection pending with a
	// cardinality-violation reason.
	Context("S2: when multiple provider ComponentComponentReleaseBindings match", func() {
		const (
			rbName   = "rb-conn-s2"
			crName   = "cr-conn-s2"
			envName  = "env-conn-s2"
		...
		It("surfaces a cardinality-violation pending connection", func() {
			...
			r := testReconcilerWithCachedClient()
			rb := reconcileUntil(r, rbName, func(rb *openchoreov1alpha1.ComponentReleaseBinding) bool {
				return hasPendingReason(rb, "multiple ComponentComponentReleaseBindings found")
			})

			By("PendingConnections surface the multiple-bindings reason")
			Expect(rb.Status.PendingConnections).To(HaveLen(1))
			Expect(rb.Status.PendingConnections[0].Reason).To(ContainSubstring("multiple ComponentComponentReleaseBindings found"))
	...
	// S3: the single provider ComponentReleaseBinding exists but is in the Undeploy state. The consumer
	...
	// S4: the provider ComponentReleaseBinding is deployed but its status advertises a different endpoint
	...
	Context("S4: when the requested endpoint is absent from provider status", func() {
		...
		It("surfaces a not-yet-resolved pending connection", func() {
			...
			setProviderEndpoints(&openchoreov1alpha1.ComponentReleaseBinding{ObjectMeta: metav1.ObjectMeta{Name: provRB}},
	...
	// S6: the provider ComponentReleaseBinding is deployed with the endpoint initially absent, so the
	...
	Context("S6: when a pending connection later resolves (project visibility)", func() {
		...
		It("transitions from pending to resolved when the provider exposes the endpoint", func() {
			...
			By("Reconciling until the not-yet-resolved pending connection is surfaced")
			r := testReconcilerWithCachedClient()
			reconcileUntil(r, rbName, func(rb *openchoreov1alpha1.ComponentReleaseBinding) bool {
				return hasPendingReason(rb, "not yet resolved")
			})
	...
	// provider ComponentReleaseBinding lives in a different project. The consumer resolves cross-project via
	...
	// S7: end-to-end projection. The provider ComponentReleaseBinding exposes the endpoint's ServiceURL up
	...
	// S8: partial resolution. Two connections, only one with a deployed provider ComponentReleaseBinding. The
	...
	Context("S8: when one of multiple connections is unresolvable", func() {
		...
		It("keeps the overall condition pending while still resolving the satisfiable connection", func() {
			...
			setProviderEndpoints(&openchoreov1alpha1.ComponentReleaseBinding{ObjectMeta: metav1.ObjectMeta{Name: provRBA}},
				[]openchoreov1alpha1.EndpointURLStatus{
					{Name: epName, ServiceURL: &openchoreov1alpha1.EndpointURL{Scheme: "http", Host: svcHost, Port: 8080}},
				})
