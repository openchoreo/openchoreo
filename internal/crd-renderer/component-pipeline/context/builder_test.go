	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if addon, ok := got["addon"].(map[string]any); ok {
				...
				if addon["instanceName"] != tt.want["addon"].(map[string]any)["instanceName"] {
					t.Errorf("BuildAddonContext() addon.instanceName = %v, want %v",
						addon["instanceName"], tt.want["addon"].(map[string]any)["instanceName"])
				}
			}
		})
	}
