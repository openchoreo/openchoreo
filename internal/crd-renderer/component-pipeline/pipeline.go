...
//
// The pipeline combines ComponentEnvSnapshot and ComponentDeployment to generate fully resolved
...
func (p *Pipeline) Render(input *RenderInput) (*RenderOutput, error) {
	...
	for _, addonInstance := range input.Snapshot.Spec.Component.Spec.Addons {
		...
		if !ok {
			return nil, fmt.Errorf("addon %s referenced but not found in snapshot", addonInstance.Name)
		}
