...
// ExtractSchema converts a field map written with the simple schema shorthand into a JSON schema.
// The types parameter provides custom type definitions that can be referenced in the fields.
...
// It splits on whitespace only. Constraints must be space-separated after the initial type|constraint split.
