...
func (s *Server) handleStream(ctx context.Context, stream net.Conn, capability string) {
	...
	var open depconnect.StreamOpen
	if err := depconnect.ReadMessage(stream, &open); err != nil {
	...
	if err != nil {
		...
		_ = depconnect.WriteMessage(stream, depconnect.StreamResult{OK: false, Error: "not authorized"})
		return
	}
	...
	if err != nil {
		...
		_ = depconnect.WriteMessage(stream, depconnect.StreamResult{OK: false, Error: "dial failed"})
		return
	}
	defer upstream.Close()

	if err := depconnect.WriteMessage(stream, depconnect.StreamResult{OK: true}); err != nil {
		s.log.Debug("stream result write failed", "key", open.Key, "error", err)
		return
	}
	s.log.Debug("stream connected", "key", open.Key, "addr", addr)
	depconnect.Pipe(stream, upstream)
}
