...
type streamAuthorizer interface {
	authorize(ctx context.Context, capability, key string) (*depconnect.AuthorizeResponse, error)
}
...
// Server terminates dep-connect tunnels and forwards each multiplexed stream to the
...
func New(cfg Config, log *slog.Logger) (*Server, error) {
	...
	if cfg.HeartbeatURL != "" && cfg.Namespace != "" {
		...
		log.Warn("dep-agent heartbeats disabled (no heartbeat URL or namespace configured); "+
...
func (s *Server) Run(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("dep-agent: listen on %s: %w", s.cfg.ListenAddr, err)
	}
...
func (s *Server) Serve(ctx context.Context, ln net.Listener) error {
	...
	s.log.Info("dep-agent listening", "addr", ln.Addr().String())
	for {
		conn, err := ln.Accept()
		if err != nil {
			if ctx.Err() != nil {
			...
			return fmt.Errorf("dep-agent: accept: %w", err)
		}
...
func (s *Server) handshake(conn net.Conn, remote string) (capability string, ok bool) {
	...
	var hello depconnect.Hello
	if err := depconnect.ReadMessage(conn, &hello); err != nil {
		s.log.Warn("handshake read failed", "remote", remote, "error", err)
		return "", false
	}
	if hello.ProtocolVersion != depconnect.ProtocolVersion {
		_ = depconnect.WriteMessage(conn, depconnect.HelloResult{OK: false, Error: "unsupported protocol version"})
		return "", false
	}
	if hello.Capability == "" {
		_ = depconnect.WriteMessage(conn, depconnect.HelloResult{OK: false, Error: "missing capability"})
		return "", false
	}
	if err := depconnect.WriteMessage(conn, depconnect.HelloResult{OK: true}); err != nil {
...
func (s *Server) rejectStream(stream net.Conn, reason string) {
	defer stream.Close()
	_ = depconnect.WriteMessage(stream, depconnect.StreamResult{OK: false, Error: reason})
}

func serverTLSConfig(certPath, keyPath string) (*tls.Config, error) {
	cert, err := tls.LoadX509KeyPair(certPath, keyPath)
	if err != nil {
		return nil, fmt.Errorf("dep-agent: load TLS keypair: %w", err)
	}
	return &tls.Config{
