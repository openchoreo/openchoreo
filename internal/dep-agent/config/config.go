...
// Package config loads the dev-tunnel agent configuration through the shared OpenChoreo
...
// OC_DEP_AGENT__AGENT__LISTEN_ADDR -> agent.listen_addr.
const EnvPrefix = "OC_DEP_AGENT"

// namespaceEnv is the downward-API variable the control plane injects into every
// provisioned dep-agent Pod. It seeds agent.namespace as the lowest-priority default,
// so a config file, a prefixed env var, or --namespace still override it.
const namespaceEnv = "POD_NAMESPACE"

// Config is the top-level configuration for dep-agent.
type Config struct {
...
func AgentDefaults() AgentConfig {
	return AgentConfig{
		...
		HandshakeTimeout:     depagent.DefaultHandshakeTimeout,
		StreamOpenTimeout:    depagent.DefaultStreamOpenTimeout,
		DialTimeout:          depagent.DefaultDialTimeout,
	}
}

// AuthorizeDefaults returns the default authorize configuration.
func AuthorizeDefaults() AuthorizeConfig {
	return AuthorizeConfig{Timeout: depagent.DefaultAuthorizeTimeout}
}

// HeartbeatDefaults returns the default heartbeat configuration.
func HeartbeatDefaults() HeartbeatConfig {
	return HeartbeatConfig{Interval: depagent.DefaultHeartbeatInterval}
}
...
//  2. Environment variables (OC_DEP_AGENT__AGENT__LISTEN_ADDR -> agent.listen_addr)
//  3. Config file (YAML)
//  4. Struct defaults
//
...
func (c *Config) ToAgentConfig() depagent.Config {
	return depagent.Config{
