...
func buildMCPHandler(svcs *testServices) (*MCPHandler, error) {
	...
	return NewMCPHandler(healthSvc, svcs.logs, svcs.platformLogs, svcs.events, svcs.metrics, svcs.alertsIncidents,
		svcs.traces, svcs.finops, svcs.insights, logger)
}
...
func TestNewMCPHandlerValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil healthService", nil, logs, platformLogs, events, metrics, alertIncidentSvc, traces, finops, deliveryInsightsSvc, logger},
		{"nil logsService", healthSvc, nil, platformLogs, events, metrics, alertIncidentSvc, traces, finops, deliveryInsightsSvc, logger},
		{"nil platformLogsService", healthSvc, logs, nil, events, metrics, alertIncidentSvc, traces, finops, deliveryInsightsSvc, logger},
		{"nil eventsService", healthSvc, logs, platformLogs, nil, metrics, alertIncidentSvc, traces, finops, deliveryInsightsSvc, logger},
		{"nil metricsService", healthSvc, logs, platformLogs, events, nil, alertIncidentSvc, traces, finops, deliveryInsightsSvc, logger},
		{"nil alertIncidentService", healthSvc, logs, platformLogs, events, metrics, nil, traces, finops, deliveryInsightsSvc, logger},
		{"nil tracesService", healthSvc, logs, platformLogs, events, metrics, alertIncidentSvc, nil, finops, deliveryInsightsSvc, logger},
		{"nil finopsService", healthSvc, logs, platformLogs, events, metrics, alertIncidentSvc, traces, nil, deliveryInsightsSvc, logger},
		{"nil deliveryInsightsService", healthSvc, logs, platformLogs, events, metrics, alertIncidentSvc, traces, finops, nil, logger},
		{"nil logger", healthSvc, logs, platformLogs, events, metrics, alertIncidentSvc, traces, finops, deliveryInsightsSvc, nil},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := NewMCPHandler(tt.health, tt.logs, tt.platformLogs, tt.events, tt.metrics,
				tt.alertIncidentService, tt.traces, tt.finops, tt.insights, tt.log)
