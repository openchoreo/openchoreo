...
var _ = Describe("ClusterTrait Webhook", func() {
	...
	Context("When creating or updating ClusterTrait under Validating Webhook", func() {
		It("Should admit clustertrait with valid parameters and envOverrides", func() {
		...
		It("Should reject clustertrait with invalid envOverrides schema syntax", func() {
			obj.Spec.Schema = openchoreodevv1alpha1.TraitSchema{
				Parameters: &runtime.RawExtension{
					Raw: []byte(`{"mountPath": "string"}`),
				},
		...
		It("Should admit clustertrait with only envOverrides (no parameters)", func() {
			obj.Spec.Schema = openchoreodevv1alpha1.TraitSchema{
				EnvOverrides: &runtime.RawExtension{
		...
		It("Should reject clustertrait with malformed envOverrides YAML", func() {
			obj.Spec.Schema = openchoreodevv1alpha1.TraitSchema{
				EnvOverrides: &runtime.RawExtension{
