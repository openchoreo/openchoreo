...
var _ = Describe("ComponentReleaseBinding Webhook", func() {
	var (
		validator Validator
		defaulter Defaulter
	)
	...
	buildRequest := func(op admissionv1.Operation, obj *openchoreodevv1alpha1.ComponentReleaseBinding, oldObj *openchoreodevv1alpha1.ComponentReleaseBinding) admission.Request {
	...
	Context("Defaulter webhook (Handle)", func() {
		It("should pass through a CREATE request unchanged when releaseName is already set", func() {
			obj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
		...
		It("should preserve releaseName from old object on UPDATE when new object has empty releaseName", func() {
			oldObj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
			oldObj.Spec.ReleaseName = "auto-release-abc"
			newObj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
	...
	Context("Validator webhook", func() {
		It("should admit ComponentReleaseBinding creation (no-op validator)", func() {
			obj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
			_, err := validator.ValidateCreate(ctx, obj)
			Expect(err).NotTo(HaveOccurred())
		})

		It("should admit ComponentReleaseBinding update (no-op validator)", func() {
			oldObj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
			newObj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
		...
		It("should admit ComponentReleaseBinding deletion (no-op validator)", func() {
			obj := &openchoreodevv1alpha1.ComponentReleaseBinding{}
