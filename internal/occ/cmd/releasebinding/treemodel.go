...
// countReachable returns the number of distinct nodes below tn. Shared
// descendants are counted once, and tn itself is never counted, even when a
// cycle leads back to it.
func countReachable(tn *treeNode) int {
	seen := map[*treeNode]bool{tn: true}
