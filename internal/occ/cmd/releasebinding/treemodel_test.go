...
func TestCountReachable_DedupsSharedDescendants(t *testing.T) {
	roots := buildForest([]gen.ResourceNode{
		testNode("a", "Deployment", "top"),
		testNode("b", "ReplicaSet", "mid-1", "a"),
		testNode("c", "ReplicaSet", "mid-2", "a"),
		testNode("d", "Pod", "shared", "b", "c"),
	})
	require.Len(t, roots, 1)
	assert.Equal(t, 3, countReachable(roots[0]), "shared Pod counted once")
}

func TestCountReachable_CycleExcludesStartNode(t *testing.T) {
	roots := buildForest([]gen.ResourceNode{
		testNode("a", "Widget", "first", "b"),
		testNode("b", "Widget", "second", "a"),
	})
	require.Len(t, roots, 1)
	assert.Equal(t, 1, countReachable(roots[0]), "a cycle back to the start node does not count it as its own descendant")
}
