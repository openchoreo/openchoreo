...
under that node — the tree there is incomplete, not empty.`,
  ...
  # Only the roots and their direct children
  occ releasebinding tree checkout-dev --namespace acme-corp --depth 2
  ...
  occ releasebinding tree checkout-dev --namespace acme-corp --kind Pod`,
	...
	flags.AddNamespace(cmd)
	cmd.Flags().Bool("watch", false, "Continuously refresh the tree")
	...
	cmd.Flags().Int("depth", 0, "Maximum tree depth to show (0 = unlimited; roots are depth 1; the cut marker's count is not filtered by --kind)")
