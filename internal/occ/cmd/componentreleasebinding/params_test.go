...
func TestGetParams_GetNamespace(t *testing.T) {
	p := GetParams{Namespace: "org-a", ComponentReleaseBindingName: "binding-1"}
	assert.Equal(t, "org-a", p.GetNamespace())
}
...
func TestDeleteParams_Getters(t *testing.T) {
	p := DeleteParams{Namespace: "org-a", ComponentReleaseBindingName: "binding-1"}
	assert.Equal(t, "org-a", p.GetNamespace())
	assert.Equal(t, "binding-1", p.GetComponentReleaseBindingName())
}

func TestDeleteParams_Getters_Empty(t *testing.T) {
	p := DeleteParams{}
	assert.Empty(t, p.GetNamespace())
	assert.Empty(t, p.GetComponentReleaseBindingName())
}
