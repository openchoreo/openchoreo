...
func TestBuildBindingOutputDirResolver(t *testing.T) {
	...
	t.Run("priority 2: bindings dir next to component (does not exist on disk)", func(t *testing.T) {
		...
		assert.Equal(t, filepath.Join(compDir, "component-release-bindings"), resolver("my-proj", "my-comp"))
	})

	t.Run("component-release-bindings dir already exists (empty): still uses component-release-bindings/", func(t *testing.T) {
		idx := index.New("/repo")

		tmpDir := t.TempDir()
		compDir := filepath.Join(tmpDir, "projects", "my-proj", "components", "my-comp")
		bindingsDir := filepath.Join(compDir, "component-release-bindings")
...
// addBinding adds a ComponentReleaseBinding resource entry to the index.
