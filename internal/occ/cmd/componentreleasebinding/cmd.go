...
func NewComponentReleaseBindingCmd(f client.NewClientFunc) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "componentreleasebinding",
		Aliases: []string{"componentcomponentreleasebindings", "rb"},
...
func newGenerateCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Generate ComponentReleaseBinding manifests for one or more components.",
...
func newListCmd(f client.NewClientFunc) *cobra.Command {
  ...
  occ componentreleasebinding list --namespace acme-corp --project online-store --component product-catalog`,
...
func newGetCmd(f client.NewClientFunc) *cobra.Command {
  ...
  occ componentreleasebinding get my-binding --namespace acme-corp`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return New(cl).Get(GetParams{
				Namespace:                   flags.GetNamespace(cmd),
				ComponentReleaseBindingName: args[0],
			})
		},
	}
	flags.AddNamespace(cmd)
...
func newDeleteCmd(f client.NewClientFunc) *cobra.Command {
  ...
  occ componentreleasebinding delete my-binding --namespace acme-corp`,
		Args:    cmdutil.ExactOneArgWithUsage(),
		PreRunE: auth.RequireLogin(),
		RunE: func(cmd *cobra.Command, args []string) error {
			cl, err := f()
