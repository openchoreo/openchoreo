...
import (
	...
	"github.com/openchoreo/openchoreo/internal/dep-agent/config"
...
func main() {
	...
	srv, err := depagent.New(cfg.ToAgentConfig(), logger)
	if err != nil {
...
func setupFlags() (*pflag.FlagSet, *cliFlags) {
	flags := pflag.NewFlagSet("dep-agent", pflag.ExitOnError)
	cli := &cliFlags{}
