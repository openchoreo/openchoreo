...
func workloadReplacements(root string, appPath string, scope string, assertionFile string) []string {
	return []string{
		...
		"{{inputs.parameters.oauth-client-assertion-file}}", assertionFile,
...
func TestGenerateWorkload_ClientAssertionReplacesClientSecret(t *testing.T) {
	script := scriptForTemplate(t, "generate-workload.yaml", "generate-workload-cr")
	env := envForTemplate(t, "generate-workload.yaml", "generate-workload-cr")
	res := runScriptWithEnv(t, script, env, map[string]string{
		"occ":  occStub,
		...
		require.NoError(t, os.MkdirAll(filepath.Join(root, "mnt-vol", "source", "service"), 0o755))
		require.NoError(t, os.MkdirAll(filepath.Join(root, "tools", "bin"), 0o755))
		require.NoError(t, os.WriteFile(filepath.Join(root, "identity-token"),
			[]byte("header.payload.signature\n"), 0o600))
	}, func(root string) []string {
		return workloadReplacements(root, "service", "", filepath.Join(root, "identity-token"))
	})

	requireScriptSuccess(t, res, "a client assertion should authenticate the workload publisher without a client secret")
	requireHasCall(t, res, "curl-form client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
		"token request must declare the RFC 7523 assertion type when an assertion file is configured")
	requireHasCall(t, res, "curl-form client_assertion=header.payload.signature",
		"assertion file contents must be sent with surrounding whitespace trimmed")
	requireNoCall(t, res, "curl-form client_secret=",
		"client secret must not be sent when an assertion file is configured")
}

func TestGenerateWorkload_MissingClientAssertionFileFailsBeforeCreate(t *testing.T) {
	script := scriptForTemplate(t, "generate-workload.yaml", "generate-workload-cr")
	env := envForTemplate(t, "generate-workload.yaml", "generate-workload-cr")
	res := runScriptWithEnv(t, script, env, map[string]string{
		"occ":  occStub,
		...
		return workloadReplacements(root, "service", "", filepath.Join(root, "absent-token"))
	})

	requireScriptExitCode(t, res, 1, "a configured but absent assertion file should fail before the workload API is called")
	requireOutputContains(t, res, "is missing or empty",
		"missing assertion file failure should name the unreadable path")
	requireNoCall(t, res, "curl POST https://auth.example.test/oauth2/token",
		"no token request may be attempted when the assertion file is unreadable")
	requireNoCallContains(t, res, "curl POST", "/api/v1/namespaces/default/workloads",
		"workload API must not be called when the assertion file is unreadable")
}
