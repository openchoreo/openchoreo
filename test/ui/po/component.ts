...
export interface BuildFromSourceInput {
  repoUrl: string;
  // Defaults to the form's own defaults when omitted (branch "main").
  branch?: string;
  appPath?: string;
  // Build workflow to pick, by metadata name. The picker renders ClusterWorkflow
  // options as "<name> (cluster)". Defaults to "dockerfile-builder".
  workflow?: string;
}
...
export interface CreateComponentInput {
  ...
  // Deployment source — exactly one of the two:
  //   image  -> "Container Image" card (deploy-from-image)
  //   source -> "Build from Source" card (git repo + build workflow)
  image?: string; // container image reference (Container Image deployment source)
  source?: BuildFromSourceInput;
  // Web Application / Service component types require at least one HTTP
  // endpoint at render time, so image-based creation adds one. Defaults to
  // 8080. Build-from-source skips the endpoint sub-form entirely — endpoints
  // are generated by the build's generate-workload step, not the wizard.
  endpointPort?: number;
  displayName?: string;
  description?: string;
}
...
export class ComponentPO {
  ...
  // Step 2: Build & Deploy — pick a deployment-source card and fill its
  // branch. The source radios have no accessible name, so the card heading is
  // the click target (MUI subtitle1 renders as an h6).
  private async fillBuildAndDeploy(input: CreateComponentInput): Promise<void> {
    if (input.source) {
      await this.fillBuildFromSource(input.source);
      return;
    }
    await this.page
  ...
  // "Build from Source" branch of the Build & Deploy step: workflow picker
  // (MUI Select labelled "Build Workflow"; ClusterWorkflow options render as
  // "<name> (cluster)" plus their description) followed by the git-source
  // fields. Workflow parameters (docker context/filePath, …) all carry
  // defaults, so they are left untouched.
  private async fillBuildFromSource(source: BuildFromSourceInput): Promise<void> {
    await this.page
      .getByRole('heading', { name: 'Build from Source', level: 6 })
      .click();
    const workflow = source.workflow ?? 'dockerfile-builder';
    await this.page.getByRole('button', { name: 'Build Workflow' }).click();
    await this.page
      .getByRole('option', {
        name: new RegExp(`${workflow} \\(cluster\\)`),
      })
      .click();
    await this.gitSourceField('Git Repository URL').fill(source.repoUrl);
    if (source.branch) {
      await this.gitSourceField('Branch').fill(source.branch);
    }
    if (source.appPath) {
      await this.gitSourceField('Application Path').fill(source.appPath);
    }
  }

  // The GitSourceField TextFields render without ids, so MUI never associates
  // their labels and the inputs have EMPTY accessible names — getByRole/
  // getByLabel can't reach them. Class selectors don't work either: this
  // bundle suffixes MUI's global classes (MuiFormControl-root-1306). Anchor
  // on the visible <label> and walk to the input in the sibling input wrapper
  // (MUI v4 FormControl renders label + input-container as siblings).
  private gitSourceField(label: string) {
    return this.page
  ...
  async create(input: CreateComponentInput): Promise<void> {
    await this.fillWizardAndSubmit(input);
  ...
  // Drive the wizard to submission and expect the scaffolder TASK to fail
  // with `errorPattern` on the task page. Use for server-side rejections that
  // only surface at action run time (e.g. duplicate component name) — the
  // wizard itself accepts the form and the failure shows on the task page.
  async createExpectingTaskError(
    input: CreateComponentInput,
    errorPattern: RegExp,
  ): Promise<void> {
    await this.fillWizardAndSubmit(input);
    await expect(
      this.page.getByText(errorPattern).first(),
    ).toBeVisible({ timeout: 60_000 });
  }

  private async fillWizardAndSubmit(input: CreateComponentInput): Promise<void> {
    ...
    await this.clickStep('Next'); // -> <template> Details
    // Build-from-source components get their workload (and endpoints) from
    // the build's generate-workload step — the Details page is left empty.
    if (!input.source) {
