...
kubectl get componentreleasebinding greeting-service-development -n default -o jsonpath='{.status.conditions}' | jq .
...
1. **Check ComponentReleaseBinding status:**
   ```bash
   kubectl get componentreleasebinding greeting-service-development -n default -o yaml
   ```
