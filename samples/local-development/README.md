...
The type declares a `client` endpoint (`spec.endpoints`), which is what makes this Resource tunnellable — `occ remote` opens one listener per declared endpoint.
