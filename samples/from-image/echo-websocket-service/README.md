...
1. Check if the ComponentReleaseBinding is ready:
   ```bash
   kubectl get componentreleasebinding echo-websocket-service-development -n default -o yaml
   ```

2. Check the ComponentReleaseBinding status conditions:
   ```bash
   kubectl get componentreleasebinding echo-websocket-service-development -n default -o jsonpath='{.status.conditions}' | jq .
   ```
