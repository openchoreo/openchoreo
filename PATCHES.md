...
| 7 | Sidecars y Workload Identity en el observability plane | `install/helm/openchoreo-observability-plane/templates/observer/observer-deployment.yaml`, `install/helm/openchoreo-observability-plane/templates/observer/observer-service-account.yaml`, `install/helm/openchoreo-observability-plane/templates/sre-agent/deployment.yaml`, `install/helm/openchoreo-observability-plane/templates/finops-agent/deployment.yaml`, `install/helm/openchoreo-observability-plane/values.yaml`, `install/helm/openchoreo-observability-plane/values.schema.json` | Cloud SQL no es alcanzable por red desde estos clusters y no puede serlo (D5.2), asi que el patron soportado es un sidecar Cloud SQL Auth Proxy. El chart no tenia forma de agregar un contenedor, ni de darle ServiceAccount a `sre-agent`/`finops-agent` —que corrian con la `default`, o sea sin identidad—, ni de anotar la SA del `observer` para Workload Identity. Se agregan `extraContainers`, `extraVolumes`, `serviceAccountName` y `observer.serviceAccountAnnotations`, todos aditivos y con default vacio. Sin esto el sidecar habia que parchearlo a mano y el proximo `helm upgrade` lo borraba | *(pendiente de abrir)* | Retirar cuando upstream acepte estas cuatro claves (o equivalentes) y el tag pinneado las incluya |
| 8 | Backstage no exige Jenkins y usa probes reales | `install/helm/openchoreo-control-plane/templates/backstage/configmap-ci.yaml`, `install/helm/openchoreo-control-plane/templates/backstage/deployment.yaml` | El chart renderizaba config/env de Jenkins aunque `backstage.externalCI.jenkins.enabled=false`, lo que bloquea portales que cargan el plugin pero no tienen Jenkins configurado. Además los probes seguían apuntando a `/healthcheck`, pero el backend actual expone `/.backstage/health/v1/liveness` y `/.backstage/health/v1/readiness` | [openchoreo/openchoreo#4542](https://github.com/openchoreo/openchoreo/pull/4542) | Retirar cuando el PR upstream esté mergeado y el tag pinneado incluya Jenkins condicionado por value y los endpoints nuevos de health |

| 8 | El hold por `retainPolicy: Retain` se loguea | `internal/controller/resourcereleasebinding/controller_finalize.go`, `internal/controller/resourcereleasebinding/controller_finalize_test.go`, `.github/workflows/idp-fork-ci.yml` | **No es un bug de comportamiento: el borrado SÍ se reconcilia.** `Reconcile` derivá a `finalize()` cuando hay `deletionTimestamp`, y `finalize()` sostiene el finalizer a propósito cuando la política efectiva es `Retain`. El problema es de diagnosticabilidad: lo hace **en silencio**. El único síntoma es un `kubectl delete` que nunca vuelve, y la condición `Finalizing/RetainHold` que lo explica es **ilegible mientras el comando cuelga** — no podés leer el status de un objeto cuyo delete no retornó. Eso es indistinguible de un controlador que ignora los borrados, y nos costó una sesión entera de trabajo diagnosticarlo mal (ver `piloto-t73.md` §4c.3). El parche agrega una línea de log en la rama `Retain` con el motivo y las dos salidas (limpiar el finalizer a mano, o `spec.retainPolicy=Delete` para cascadear), más dos guards en CI | [openchoreo/openchoreo#4541](https://github.com/openchoreo/openchoreo/pull/4541) | Retirar cuando upstream loguee el hold por `Retain` (esta línea u otra equivalente que nombre `retainPolicy=Retain` en la rama de finalize) y el tag pinneado la incluya. ⚠️ Los **dos specs de guard** no se retiran con el parche: `dispatches to the finalize path on DeletionTimestamp` y `logs why it is holding the finalizer when retainPolicy=Retain` se quedan aunque upstream mergee, porque cubren la regresión, no el parche |
